...
37. **AI 重写决策（2026-09-21）**：现有 AI（GameController 里协程 + DecideOrders 未被执行 + TryEndPhase 直接跳过 AI）不满足联网需求，需重写为「服务端输入源」：AiDecider（Model/Ai，纯 C# 决策，输入 GameState+playerId → 输出 BaseOrder，可单测） + AiDriver（Controller/Ai，服务端驱动循环，只做节流，不等动画）；人类与 AI 共用 ExecuteAndBroadcast(order)（执行 + 广播）；客户端不感知 AI，只收快照+Hint；AI 只认 playerId，便于掉线接管复用。
38. **动画队列暂缓（2026-09-21）**：当前不实现客户端 Hint 队列。做法是先依赖单个操作的动画连贯性、AI stepDelay 和服务端一次操作一次广播来避免重叠；若后续出现连续 AI 行动或长动画重叠，再回头实现动画队列。动画队列不是当前状态同步主线的阻塞项。
